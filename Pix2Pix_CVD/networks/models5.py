...
class Encoder(nn.Module):
    ...
    def forward(self, x):  # (N, 3, 256, 256)
        feat2 = self.S1S2(x)  # (N, 16, 128, 128)
        feat3 = self.S3(feat2)  # (N, 64, 64, 64)
        feat4 = self.S4(feat3)  # (N, 128, 32, 32)
        feat5 = self.S5(feat4)  # (N, 128, 32, 32)
...
class DetailBranch(nn.Module):
    ...
    def forward(self, x):  # (N, 3, 256, 256)
        feat = self.S1(x)  # (N, 64, 128, 128)
        feat = self.S2(feat)  # (N, 128, 64, 64)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, e, d):
        '''e: upsampled encoder feature (N, 128, 64, 64)
           d: Detail feature (N, 128, 64, 64)'''
        b, c, h, w = e.shape
...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.output_conv = nn.ConvTranspose2d(
            16, 3, 2, stride=2, padding=0, output_padding=0, bias=False)
...
class GeneratorNet(nn.Module):
    ...
    def forward(self, input): # input : (N, 3, 256, 256)
        encoded_feature = self.encoder(input)  # (N, 128, 32, 32)
        encoded_feature = self.up(encoded_feature)  # (N, 128, 64, 64)
        detail_feature = self.detail_branch(input)  # (N, 128, 64, 64)

        # CA feature (query=encoder feature, key=detail feature)
        ca_feature_ed, _ = self.fuse_transform(encoded_feature, detail_feature)  # (N, 128. 64, 64)

        return self.decoder.forward(ca_feature_ed)  # (N, 3, 256, 256)
