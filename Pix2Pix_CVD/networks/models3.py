...
class Up(nn.Module):
    """Upscaling(Bilinear interpolation or Conv2dTranspose) then double conv"""
...
class GeneratorUNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=3, bilinear=True):  # upsampling with bilinear interpolation
    ...
    def forward(self, x):  # (N, 3, 256, 256)
        x1 = self.inc(x)  # (N, 64, 256, 256)
        x2 = self.down1(x1)  # (N, 128, 128, 128)
        x3 = self.down2(x2)  # (N, 256, 64, 64)
        x4 = self.down3(x3)  # (N, 512, 32, 32)
        x5 = self.down4(x4)  # (N, 1024, 16, 16)
        
        x = self.up1(x5, x4)  # (N, 512, 32, 32)
        x = self.up2(x, x3)  # (N, 256, 64, 64)
        x = self.up3(x, x2)  # (N, 128, 128, 128)
        x = self.up4(x, x1)  # (N, 64, 256, 256)
        
        logits = self.outc(x)  # (N, 3, 256, 256)
